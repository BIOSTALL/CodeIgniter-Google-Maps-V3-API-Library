...
class Googlemaps {
    ...
    var $geocodeCaching = TRUE;     // If set to TRUE will cache any geocode requests made when an address is used instead of a lat/long. Requires DB table to be created (see documentation)
        ...
        if ($this->center == "auto") { // if wanting to center on the users location
            $this->output_js_contents .= '
                        if (google.loader.ClientLocation) {
                            var myLatlng = new google.maps.LatLng(google.loader.ClientLocation.latitude,
        ...
        if ($this->places) {
            $this->output_js_contents .= 'function placesCallback(results, status) {
				if (status == google.maps.places.PlacesServiceStatus.OK) {
					...
					';
            if ($this->zoom == "auto") {
                $this->output_js_contents .= 'fitMapToBounds();';
            }
            $this->output_js_contents .= '
				}
			}
    ...
    function get_lat_long_from_address($address) {
        ...
        if ($this->geocodeCaching) { // if caching of geocode requests is activated
            $CI = & get_instance();
            $CI->load->database();
            $CI->db->select("latitude,longitude, formatted_address");
            ...
            if ($query->num_rows() > 0) {
                $row = $query->row(); 
                return array($row->latitude, $row->longitude, $row->formatted_address);
            }
        }
